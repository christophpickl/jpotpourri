 ...
 * @author Tim Boudreau; http://javabyexample.wisdomplug.com/java-concepts/34-core-java/59-tips-and-tricks-for-jtree-jlist-and-jcombobox-part-i.html
...
public final class ViewTooltips extends MouseAdapter implements MouseMotionListener {
	...
	public static void register(JList list) {
	...
	private static void registerSafeType(JComponent comp) {
		if (INSTANCE == null) {
	...
	private static final Rectangle[] getRects(final Rectangle bds,
		...
		if (vis.contains(bds)) {
			...
			if (bds.x < vis.x && bds.x + bds.width > vis.x + vis.width) {
				...
				new Rectangle(vis.x + vis.width, bds.y, (bds.x + bds.width)
						- (vis.x + vis.width), bds.height)
	...
	private static final class Hider implements ChangeListener,
		...
		private void attach() {
...

			{
				...
				pane.getHorizontalScrollBar().getModel()
		...
		private void detach() {
...

			{
				...
				pane.getHorizontalScrollBar().getModel().removeChangeListener(
						this);
...

			{
				...
				pane.getVerticalScrollBar().getModel().removeChangeListener(
						this);
